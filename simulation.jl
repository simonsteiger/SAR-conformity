...
# ╔═╡ 7b07dd2e-cfec-11ef-2539-03fb5db356b2
using Distributions, Random, CairoMakie, Chain, LinearAlgebra

# ╔═╡ 796da14d-9572-4ff3-af2d-10808561c6d6
using Turing, DataFrames

# ╔═╡ a8834421-9444-42b2-8dbc-ca32705893a6
using PlutoUI

# ╔═╡ f2afc21b-9537-4557-9345-0f98bec79c25
md"""
# SAR Simulation
"""

# ╔═╡ d81da8e1-dc4b-43b6-8840-a1df74e5406e
md"""
## Visualization
"""

# ╔═╡ aac398c7-32ed-427b-8420-2503ce97cf74
md"""
# Code
"""

# ╔═╡ fa4fd725-24ea-4a3b-bfc0-575d3037a109
begin
	c_in = @bind c PlutoUI.Slider(1:1:5, default=1)
	z_in = @bind z PlutoUI.Slider(0:3:9, default=3)
	γ0_in = @bind γ0 PlutoUI.Slider(-1:0.2:1, default=-0.8)
	γ1_in = @bind γ1 PlutoUI.Slider(-3:0.5:0, default=-1.5)
	σ0_in = @bind σ0 PlutoUI.Slider(-1:0.2:1, default=-0.8)
end;

# ╔═╡ 09808536-96f4-44ef-b0e1-359ada0428a2
md"""
## Parameter settings
...
Slope of Arrhenius power law

$z_in

### Set `γ_0`

Effect size of atoll-level conformity-increasing covariate `X_i`

$γ0_in

### Set `γ_1`

Effect size of islet-level conformity-increasing covariate `A_j`
...
# ╔═╡ a839d0c7-e4b4-40d8-aa3c-509f6dcc9c4d
md"""
The current parameter configuration is:
...
Homoscedastic variance and no variance at all:
"""

# ╔═╡ a031acb2-2f10-4474-9042-f512e250b6e2
rng = Xoshiro(521)

# ╔═╡ 88b8ff35-0398-48ed-930a-9c1ef275f6a6
n_atolls = 1

# ╔═╡ cfbca297-29a0-4e06-b477-e81112f84a11
islets_per_atoll = 10_000

# ╔═╡ 7732d9d2-34a2-44d7-90d3-7c45e1e4ecd4
A_zlog = rand(rng, Normal(0, 1), islets_per_atoll)

# ╔═╡ 587d6935-77b4-40da-8019-bf041a558e11
S_zlog = log.(c) .+ A_zlog .* z

# ╔═╡ 6d2aba21-ad47-47dc-8e4c-21bd60e695f9
σ_0 = -1.0

# ╔═╡ 5afeed18-51ff-4da3-88da-10ce37b1b394
X = range(-1.0, 1.0, length=4)

# ╔═╡ a6281568-95b7-4179-9d3f-05de3824298e
σ_0i = map(X_i -> exp(σ_0 + γ0 * X_i), X)

# ╔═╡ f9a1647c-acc2-486c-ae12-df0332a4832e
γ_1 = range(γ1, 0; length=4)

# ╔═╡ ee7f0073-df42-4025-bec0-b7ddd4b534ac
Γ = collect(Iterators.product(σ_0i, γ_1))

# ╔═╡ f454605b-77aa-4f91-a093-1bc39e4d001d
richness = map(Γ) do (σ_0i, γ_1i)
	σ_i = exp.(σ_0i .+ γ_1i .* A_zlog)
	rand(rng, MvNormal(S_zlog, σ_i))
end

# ╔═╡ 119eaf07-76e7-4b44-8374-791b9bf3ffc7
indices24 = rand(rng, 1:islets_per_atoll, 24)

# ╔═╡ 966cfc7c-48e2-4c7b-af33-153e34ae9795
richness_homoscedastic = rand(rng, MvNormal(S_zlog[indices24], I))

# ╔═╡ 040f2964-1260-49dc-a314-a60c9870a0be
richness_zeronoise = S_zlog[indices24]

# ╔═╡ 553b05c2-f66e-4f0e-9b32-16b8f5090542
standardise(x, μ, σ) = (x - μ) / σ

# ╔═╡ 978ab968-0cd3-4166-9cee-69533a4a3baa
standardise(x) = (x .- mean(x)) ./ std(x)

# ╔═╡ d293acfb-cd21-4f1b-a941-5c971867f581
let
	fig = Figure(size=(600,400))
	μ_rich = mean(reduce(vcat, richness))
	σ_rich = std(reduce(vcat, richness))
	for (i, r) in enumerate(richness)
		zr = standardise.(r, μ_rich, σ_rich)
		ax = Axis(fig[fldmod1(i, 4)...], limits=((-3.3, 3.3), (-2.0, 2.0)))
		scatter!(A_zlog, zr, strokecolor=(:darkred, 0.9), strokewidth=0, color=(:darkred, 0.4), markersize=1.6)
		hidedecorations!(ax)
	end
	# save(joinpath(@__DIR__, "sim_sar.svg"), fig)
	fig
end

# ╔═╡ 96b2fabb-ba32-4f75-9c4b-ff0efa1698a4
md"""
## Model
"""

# ╔═╡ dc9b07c9-0ca8-41cc-a7a4-fb413f5fcf49
@model function lm(x, y)
	α ~ Normal(0, 1)
	β ~ Normal(0, 1)
	σ² ~ InverseGamma(3, 2)
	μ = @. α + β * x
...
# ╔═╡ 77079031-d053-4b04-a61a-c0e253a51e32
m = lm(A_zlog[indices24], richness_homoscedastic)

# ╔═╡ d4d14379-54c5-4821-a2e8-a9dcff865040
chn = sample(m, NUTS(), MCMCThreads(), 1000, 4)

# ╔═╡ 88562a3c-a18c-4fc7-9446-c42c92f06887
sim(α, β, x) = @. α + β * x

# ╔═╡ d357c8df-6c90-49f2-a7da-96c3e05b023e
preds = [
	quantile(sim(chn[:, "α", :], chn[:, "β", :], i), [0.025, 0.5, 0.975]) 
	for i in -3:0.1:3
]

# ╔═╡ 39b6511a-8fca-45d0-928b-02b3d996e441
@model function dm(x)
	α ~ Normal(0, 1)
	β ~ Normal(0, 1)
	μ = @. α + β * x
...
# ╔═╡ cb4d721a-320b-4bf5-bb61-cd49c447c876
m_dm = dm(A_zlog) | (y=richness[1],)

# ╔═╡ ea450353-a7b1-4028-b1a5-d5e76cb2b625
chn_dm = sample(m_dm, NUTS(), 1000)

# ╔═╡ 598438a9-cfed-40df-b362-febd983063da
gq = generated_quantities(m_dm, chn_dm)

# ╔═╡ 1423f53d-ec38-480f-b002-25a245c6b380
preds_dm = [
	quantile(sim(chn_dm[:, "α", :], chn[:, "β", :], i), [0.025, 0.5, 0.975]) 
	for i in -3:0.1:3
]

# ╔═╡ d0ee78c3-e52d-4377-9f77-5f270c7592e3
sdim(a::Symbol) = x -> map(i -> getproperty(i, a), x)

# ╔═╡ 48c3ede9-22a8-41cb-a79d-e226f29e0eef
sdim(a) = x -> map(i -> i[a], x)

# ╔═╡ 368b0634-b7b1-4999-a613-8da40d0fc3b1
let
	f = Figure(size=(680, 220))
	ax1 = Axis(f[1, 1])
	ax2 = Axis(f[1, 2])
	hidedecorations!.([ax1, ax2])

	band!(ax2, -3:0.1:3, sdim(1)(preds), sdim(3)(preds), color=colorant"#E3E3E3")
	scatter!(ax2, A_zlog[indices24], richness_homoscedastic, strokecolor=(:darkred, 0.9), strokewidth=1.5, color=:transparent)
	lines!(ax2, -3:0.1:3, sdim(2)(preds), color=:black)
	lines!(ax1, sort(A_zlog[indices24])[[begin, end]], sort(richness_zeronoise)[[begin, end]], color=:black)
	scatter!(ax1, A_zlog[indices24], richness_zeronoise, strokecolor=(:darkred, 0.9), strokewidth=1.5, color=:transparent)

	# save(joinpath(@__DIR__, "sim_sar_homosced_zeronoise.svg"), f)

	f
end

# ╔═╡ 131eabd9-2572-4453-b601-3555be3c1523
μ = hcat(vec(sdim(:μ)(gq))...)

# ╔═╡ eac72f6f-6d7a-4625-8c3a-62033533f3b7
function sim_dm(model, chain)
	gq = generated_quantities(model, chain)
	μ = hcat(vec(sdim(:μ)(gq))...)
	σ = hcat(vec(sdim(:σ)(gq))...)
	y_pred = [rand(MvNormal(μ_i, σ_i)) for (μ_i, σ_i) in zip(eachcol(μ), eachcol(σ))]
	return hcat(y_pred...)
end

# ╔═╡ e0c1e83c-5d91-407f-8c14-607ecdb1ed2f
y_pred = sim_dm(m_dm, chn_dm)

# ╔═╡ f9e299d2-7090-4eba-95e2-115367985b90
let
	f = Figure()
	I = sortperm(A_zlog)
	for (i, rich) in enumerate(richness)
		ax = Axis(f[fldmod1(i, 4)...])
		m_dm = dm(A_zlog) | (y=rich,)
		chn_dm = sample(m_dm, NUTS(), MCMCThreads(), 1000, 4)
		gq = generated_quantities(m_dm, chn_dm)
		y_pred = sim_dm(m_dm, chn_dm)
		qs = quantile.(eachrow(y_pred[I, :]), Ref([0.05, 0.5, 0.95]))
		band!(A_zlog[I], sdim(1)(qs), sdim(3)(qs), color=colorant"#E3E3E3")
		lines!(A_zlog[I], sdim(2)(qs), color=:darkred)
		xlims!(-3, 3)
		ylims!(-25, 25)
		hidedecorations!(ax)
	end
...
# ╔═╡ ae3aea22-7dee-4732-8689-e0efe8daf0d3
...
manifest_format = "2.0"
project_hash = "d6441f6fa018c71a85efa0df907c9c74ef7e4659"
...
git-tree-sha1 = "0f842be484159f158958355088f6cf3d674e2ded"
...
git-tree-sha1 = "e5fde37ee23176902a9826f71abf0b9913f02b10"
