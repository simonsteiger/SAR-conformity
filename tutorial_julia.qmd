...
A common way to check if the model captured the data-generating process well is to use the model parameters to "retrodict" the data.
If the retrodicted data systematically mismatch the real data, we have failed to capture an important part of the data-generating process.
Depending on the severity of this mismatch and the nature of our research question, this might prompt us to go back and refine the model.

```{julia}
#| output: true
preds = @chain predict(model, chn) begin
	DataFrame(_)
	combine(_, names(_, Float64) .=> (x -> Ref(quantile(x, [0.1, 0.5, 0.9]))) => identity)
	stack(_, All())
	_[sortperm(area), :]
end

q10, q50, q90 = [getindex.(preds.value, i) for i in 1:3]
yerror = (q50 .- q10, q90 .- q50)

p = plot()
scatter!(sort(area), richness[sortperm(area)], label=false)
scatter!(sort(area), q50, yerror=yerror, label=false)
p
```
